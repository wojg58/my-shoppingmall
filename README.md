...
3. **환경변수 설정**
   ...
   > **💡 팁**: 
   > - 클라이언트 키와 시크릿 키를 정확히 복사하여 붙여넣으세요
   > - 따옴표는 생략해도 됩니다
   > - 키 앞뒤에 공백이 없도록 주의하세요

> **⚠️ 주의사항**: 
> - **테스트 모드**: 실제 결제가 발생하지 않습니다. 안전하게 테스트할 수 있습니다.
> - **프로덕션**: 실제 서비스 배포 시에는 라이브 키를 사용해야 합니다.
> - **보안**: `TOSS_SECRET_KEY`는 서버 사이드에서만 사용하며, **절대 공개하지 마세요**.
> - **키 형식**: 테스트 키는 `test_ck_` 또는 `test_sk_`로 시작하고, 라이브 키는 `live_ck_` 또는 `live_sk_`로 시작합니다.
